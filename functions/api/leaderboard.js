// Cloudflare Pages Function — deployed automatically at /api/leaderboard when this repo is
// hosted on Cloudflare Pages with a KV namespace bound to LEADERBOARD_KV (Pages project →
// Settings → Functions → KV namespace bindings). See README.md for the exact setup steps.
//
// Storage model: one JSON array per UTC day, key "leaderboard:YYYY-MM-DD", holding the top 25
// entries sorted by streak. Simple and cheap (fits comfortably in KV's free tier), at the cost
// of a small race window if two people submit at the exact same instant — acceptable for a
// casual daily leaderboard. Entries expire after 2 days so old boards clean themselves up.
